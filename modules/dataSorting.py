...
def get_crime(coordinate, user_choice, radius):
    ...
    sort_date = []
    sort_category = []
    #Creating a list to store only the files that we want to open
    accepted_files = ["2020-01-devon-and-cornwall-street.csv", "2020-02-devon-and-cornwall-street.csv", "2020-03-devon-and-cornwall-street.csv", "2020-04-devon-and-cornwall-street.csv", "2020-05-devon-and-cornwall-street.csv", "2020-06-devon-and-cornwall-street.csv", "2020-07-devon-and-cornwall-street.csv", "2020-08-devon-and-cornwall-street.csv", "2020-09-devon-and-cornwall-street.csv", "2020-10-devon-and-cornwall-street.csv", "2020-11-devon-and-cornwall-street.csv", "2020-12-devon-and-cornwall-street.csv"]
    ...
    if exists: # Aka if exists == True
        for root,dirs,files in os.walk(target_path):
            for file in files: #Looping through each file in the directory
                if file in accepted_files: #This if makes sure that we only open the file that we want to open
                    ...
                    with open(access_file, 'r') as cur_file:
                        ...
                        for line in csv_reader:
                            ...
                            try:
                                ...
                                pass
                else: #If the file is not what we want to open, we tell the user that
                    print(f"This file ({file}) is ignored as it is not accepted")
            if user_choice == "1": #If user wants to sort by distance
        ...
        #If the path structure isn't right or the path doesn't exist, we tell the user
